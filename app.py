...
# Page configuration
st.set_page_config(
    page_title="🛡️ AI Adware Detector",
    page_icon="🛡️",
    layout="wide",
...
def generate_ai_explanation(app_data, prediction, confidence, adware_proba):
    ...
    explanation = "## 🤖 AI Security Analysis\n\n"

    # 1. Threat Assessment
    explanation += "### 1️⃣ Threat Assessment\n\n"
    if prediction == 1:
        if adware_proba > 0.95:
            explanation += f"⚠️ **HIGH RISK**: This application exhibits strong adware characteristics with **{confidence:.1%} confidence**. "
            explanation += "The behavioral patterns closely match known adware samples in our dataset.\n\n"
        else:
            explanation += f"⚠️ **MODERATE RISK**: This application shows adware indicators with **{confidence:.1%} confidence**. "
            explanation += "Multiple suspicious patterns detected.\n\n"
    else:
        if adware_proba < 0.05:
            explanation += f"✅ **LOW RISK**: Application appears benign with **{confidence:.1%} confidence**. "
            explanation += "Patterns align with legitimate applications.\n\n"
        else:
            explanation += f"✅ **LOW-MODERATE RISK**: Likely benign with **{confidence:.1%} confidence**.\n\n"

    # 2. Behavioral Analysis
    explanation += "### 2️⃣ Behavioral Analysis\n\n"
    ...
    if network_op > 10 and http_clients > 5:
        explanation += "- 🔴 **Excessive network communication** combined with multiple HTTP clients suggests "
        explanation += "potential ad fetching or data exfiltration behavior\n"

    if show_method > 50:
        explanation += "- 🔴 **Unusually high UI display activity** indicates aggressive ad display patterns\n"

    if prediction == 0:
        explanation += "- ✅ Application behavior aligns with typical benign apps in dataset\n"
        explanation += "- ✅ Resource usage within expected ranges\n"

    explanation += "\n"

    # 3. Technical Indicators
    explanation += "### 3️⃣ Technical Indicators\n\n"
    ...
    # 4. Recommendation
    explanation += "### 4️⃣ Recommendation\n\n"

    if prediction == 1 and adware_proba > 0.95:
        explanation += "🚫 **BLOCK**: Immediate blocking recommended. High-confidence adware detection.\n"
    elif prediction == 1:
        explanation += "⛔ **QUARANTINE**: Isolate for further analysis before final decision.\n"
    elif adware_proba < 0.10:
        explanation += "✅ **ALLOW**: Safe to proceed with normal usage.\n"
    else:
        explanation += "👁️ **MONITOR**: Likely safe but periodic checks recommended.\n"
...
def main():
    # Header
    st.markdown('<h1 class="main-header">🛡️ AI-Powered Adware Detection System</h1>', unsafe_allow_html=True)

    st.markdown("""
    <div style='text-align: center; padding: 1rem; background-color: #f8f9fa; border-radius: 0.5rem; margin-bottom: 2rem;'>
        <p style='font-size: 1.2rem; margin: 0;'>
            🧠 <strong>99.63% F1-Score</strong> Neural Network •
    ...
    with st.sidebar:
        ...
        analysis_mode = st.radio(
            ...
            ["📱 Single App", "📊 Batch Processing", "📈 Feature Analysis", "ℹ️ About"]
        )
    ...
    elif analysis_mode == "📈 Feature Analysis":
...
def show_single_app_analysis(model, scaler, features):
    ...
    st.header("📱 Single App Detection")
    ...
    with col1:
        ...
        input_method = st.tabs(["📁 Upload CSV", "⌨️ Manual Input", "🎲 Use Sample"])

        with input_method[0]:
            st.info("📤 Upload a CSV file with app features")
            ...
            if uploaded_file is not None:
                try:
                    ...
                    if len(df) > 1:
                        st.warning(f"⚠️ File contains {len(df)} apps. Analyzing first app. Use 'Batch Processing' for multiple apps.")
        ...
        with input_method[1]:
            st.info("⌨️ Enter feature values manually")
        ...
        with input_method[2]:
            st.info("🎲 Use pre-loaded sample app for testing")

            col_sample1, col_sample2 = st.columns(2)

            with col_sample1:
                if st.button("🔴 Test Adware Sample", type="primary"):
            ...
            with col_sample2:
                if st.button("✅ Test Benign Sample"):
...
def analyze_app(app_data, model, scaler, features):
    ...
    # Main result card
    result_label = "🔴 ADWARE DETECTED" if prediction == 1 else "✅ BENIGN"
    ...
    behaviors = []
    if network_op > 10:
        behaviors.append(("⚠️ High network activity", f"{network_op:.0f} operations", "warning"))
    if http_clients > 5:
        behaviors.append(("⚠️ Multiple HTTP clients", f"{http_clients:.0f} clients", "warning"))
    if show_method > 50:
        behaviors.append(("⚠️ Excessive UI displays", f"{show_method:.0f} show methods", "warning"))

    if behaviors:
        for label, value, type in behaviors:
            st.warning(f"{label}: **{value}**")
    else:
        st.success("✅ No suspicious behaviors detected")
...
def show_batch_processing(model, scaler, features):
    ...
    st.info("📤 Upload a CSV file with multiple apps to analyze them all at once")
...
def show_about_page():
    ...
    with col1:
        st.markdown("""
        ## 🛡️ AI-Powered Android Adware Detection
        ...
        - **🧠 Neural Network Model**: State-of-the-art deep learning
        - **🤖 AI Explanations**: Understand every decision
        - **⚡ Real-time Detection**: Instant analysis
        - **📊 Batch Processing**: Analyze multiple apps
